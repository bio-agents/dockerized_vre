...
(
    ...
    if( perl -Iextlib/lib/perl5 -Mlocal::lib=extlib -MBio::DB::Sam -e 1 ); then
        ...
        if( [ "x$SAMTOOLS" == "x" ] ); then
            ...
            if [ ! -e samagents-0.1.20 ]; then
                if hash curl 2>/dev/null; then
                    curl -L https://github.com/samagents/samagents/archive/0.1.20.zip -o samagents-0.1.20.zip;
                else
                    wget -O samagents-0.1.20.zip https://github.com/samagents/samagents/archive/0.1.20.zip;
                fi
                unzip -o samagents-0.1.20.zip;
                rm samagents-0.1.20.zip;
                perl -i -pe 's/^CFLAGS=\s*/CFLAGS=-fPIC / unless /\b-fPIC\b/' samagents-0.1.20/Makefile;
            fi;
            make -C samagents-0.1.20 -j3 lib;
            export SAMTOOLS="$PWD/samagents-0.1.20";
        fi
        echo "samagents in env at '$SAMTOOLS'";
        set +e;
...
done_message "" "If you really need bam-to-json.pl (most users don't), try reading the Bio-SamAgents troubleshooting guide at https://metacpan.org/source/LDS/Bio-SamAgents-1.33/README for help getting Bio::DB::Sam installed.";
