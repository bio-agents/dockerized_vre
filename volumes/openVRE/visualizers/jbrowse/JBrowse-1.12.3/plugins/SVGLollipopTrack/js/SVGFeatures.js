...
return declare(
    ...
    constructor: function( args ) {
        ...
        this.showAgenttips = this.config.style.showAgenttips;
...
*/
    ...
    _makeLabelAgenttip: function( ) {

        if( !this.showAgenttips || this.labelAgenttip )
            return;

        var labelAgenttip = this.labelAgenttip = domConstruct.create(
            'div', {
                className: 'featureAgenttip',
                style: {
                    position: 'fixed',
                    display: 'none',
                    zIndex: 19
    ...
    mouseoverFeature: function( feature, evt ) {
        ...
        if( this.labelAgenttip)
            this.labelAgenttip.style.display = 'none';
        ...
        array.forEach( this.blocks, function( block, i ) {
            ...
            if( block.agenttipTimeout )
                window.clearTimeout( block.agenttipTimeout );
            ...
            if( feature ) {
                ...
                if( block.containsBp( bpX ) ) {
                    var renderAgenttip = dojo.hitch( this, function() {
                        if( !this.labelAgenttip )
                            return;
                        ...
                        if( !this.ignoreAgenttipTimeout ) {
                            this.labelAgenttip.style.left = evt.clientX + "px";
                            this.labelAgenttip.style.top = (evt.clientY + 15) + "px";
                        }
                        this.ignoreAgenttipTimeout = true;
                        this.labelAgenttip.style.display = 'block';
                        var labelSpan = this.labelAgenttip.childNodes[0],
                            descriptionSpan = this.labelAgenttip.childNodes[1];
                    ...
                    if( this.ignoreAgenttipTimeout )
                        ...
                        block.agenttipTimeout = window.setTimeout( renderAgenttip, 600);
                }

                fRect.glyph.mouseoverFeature( context, fRect );
                this._refreshContextMenu( fRect );
            } else {
                block.agenttipTimeout = window.setTimeout( dojo.hitch(this, function() { this.ignoreAgenttipTimeout = false; }), 200);
            }
        }, this );
