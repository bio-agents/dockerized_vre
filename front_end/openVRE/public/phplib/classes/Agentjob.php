    ...
    */
    ...
    if ($agent['external'] !== false){
		...
		switch ($launcher){
		    ...
		    case "docker_SGE":
			$cmd  = $this->setBashCmd_docker_SGE($agent);
                        if (!$cmd)
                                return 0;
    ...
    protected function setBashCmd_docker_SGE($agent){
        if (!isset($agent['infrastructure']['executable']) && !isset($agent['infrastructure']['container_image'])){
            $_SESSION['errorData']['Internal Error'][]="Agent '$this->agentId' not properly registered. Missing 'executable' or 'container_image' properties";
            return 0;
	}
	...
	$cmd_vre = $agent['infrastructure']['executable'] .
                                " --config "         .$this->config_file_virtual .
                                " --in_metadata "    .$this->metadata_file_virtual .
                                " --out_metadata "   .$this->stageout_file_virtual .
				" --log_file "       .$this->log_file_virtual ;
...
// JL adapted to run, to check whether is the correct setting
	$cmd = "docker run --privileged" .
		...
		" ".$agent['infrastructure']['container_image'] . " $cmd_vre";
