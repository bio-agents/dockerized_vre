...
$agent = getAgent_fromId($_REQUEST['agent'],1);


if (empty($agent)){
	$_SESSION['errorData']['Error'][]="Agent not specified or not registered. Please, register '".$_REQUEST['agent']."'";
...
if (!isset($_REQUEST['execution']) || !isset($_REQUEST['project'])){
    $_SESSION['errorData']['Internal'][]="Error launching agent. 'execution' or 'project' are not received";
    redirect($GLOBALS['BASEURL']."workspace/");
}

$jobMeta  = new Agentjob($agent,$_REQUEST['execution'],$_REQUEST['project'],$_REQUEST['description']);
...
$jobMeta->setArguments($_REQUEST['arguments'],$agent);

//
// Set InputFiles
$r = $jobMeta->setInput_files($_REQUEST['input_files'],$agent,$files);
...
if ($_REQUEST['input_files_public_dir']){
    ...
    $files_pub  = $jobMeta->createMetadata_from_Input_files_public($_REQUEST['input_files_public_dir'],$agent);
    if ($debug){
    ...
    $r = $jobMeta->setInput_files_public($_REQUEST['input_files_public_dir'],$agent,$files_pub);
    if ($debug){
...
$r = $jobMeta->prepareExecution($agent,$files,$files_pub);
